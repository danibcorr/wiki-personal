...
Un buen _commit_ es aquel que contiene exclusivamente los cambios relacionados con una
única tarea o propósito concreto. Evitar el uso indiscriminado de `git add .` y, en su
lugar, seleccionar cuidadosamente qué cambios se incluyen —por ejemplo, mediante
`git add -p`— contribuye a mantener un historial limpio y comprensible. Una práctica
recomendable consiste en crear _tickets_ específicos en un gestor de tareas (como Jira)
y asociar cada _commit_ a su _issue_ correspondiente, creando ramas dedicadas a partir
de ellos. Además, los mensajes de _commit_ deben ser concisos, idealmente de 80
caracteres o menos, para facilitar su lectura en el historial.
...
firma, basta con recrearlos para que aparezcan firmados:

???+ example "Ejemplo: Firmar todos los commits"
...
los commits existentes sin perder los cambios:

???+ example "Ejemplo: Reescribir historial eliminando commits no firmados"
...
???+ example "Ejemplo: Restablecer a un commit firmado"
