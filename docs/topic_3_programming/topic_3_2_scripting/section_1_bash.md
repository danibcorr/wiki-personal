...
permitir que el sistema lo ejecute como un programa, es necesario otorgarle permisos de
ejecución y posteriormente invocarlo desde la terminal.

???+ example "Ejemplo"

    El comando `chmod` cambia los permisos de un archivo, y la opción `+x` añade el permiso de ejecución:

    ```bash linenums="1"
    chmod +x script.sh
    ```

    Después de ejecutar este comando, es posible ejecutar el script desde la terminal directamente:
...
???+ example "Ejemplo: Script de configuración de un entorno de desarrollo"
